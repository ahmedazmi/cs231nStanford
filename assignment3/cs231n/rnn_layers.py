...
#    pass
    ...
    dq = dnext_h * (1 - next_h**2)  # shape (N, H)
    dx = np.dot(dq, Wx.T)    #shape (N, D)
    dprev_h = np.dot(dq, Wh.T)  # shape (N, H)
    dWx = np.dot(x.T, dq)     #shape (D, H)
...
#    pass
    ...
    for t in range(T):
        ...
        h.append(h_t)   # h will not include h0
...
#    pass
    ...
    for t in range(T-1,-1,-1):
        dx[:,t,:], dht, dWxt, dWht, dbt = rnn_step_backward(dht + dh[:,t,:], cache[t])
        dWx += dWxt
        dWh += dWht
        db  += dbt
    dh0 = dht
...
#    pass
    ...
    activation = np.dot(prev_h, Wh) + np.dot(x, Wx) + b     # shape (N, 4H)
    
    ai = activation[:, 0*H:1*H]     #(N, H)
    af = activation[:, 1*H:2*H]     #(N, H)
    ao = activation[:, 2*H:3*H]     #(N, H)
    ag = activation[:, 3*H:4*H]     #(N, H)
